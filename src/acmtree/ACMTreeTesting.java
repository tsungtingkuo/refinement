...
public class ACMTreeTesting {
	...
	public static void main(String[] args) throws Exception {
		...
		ACMTreeTesting.run(fileSuffix, singleFeature, alpha, beta, gamma, semanticThreshold, newAsParent, method, validAnswerOnly, testingNew98, keywordNumber, adjustScore, adjustSimilarity, view, suggestRank, 0, enrichment, features);
	}
	
	// For simplification
	public static void run(int method) throws Exception{
		double alpha = 0.0;					// Structural similarity computation
		double beta = 0.0;					// Semantic similarity adjustment
		...
		run(fileSuffix, singleFeature, alpha, beta, gamma, semanticThreshold, newAsParent, method, validAnswerOnly, testingNew98, keywordNumber, adjustScore, adjustSimilarity, view, suggestRank, 1211100, enrichment, features);
	}

	public static String run(String fileSuffix, boolean singleFeature, double alpha, double beta, double gamma, double semanticThreshold, boolean newAsParent, int method, boolean validAnswerOnly, boolean testingNew98, int keywordNumber, int adjustScore, int adjustSimilarity, int view, int suggestRank, int trainingDataSize, boolean enrichmentUsed, int featuresUsed) throws Exception {
		...
		for(int s=0; s<repeatNumber; s++) {
			...
			for(int i=iStart; i<iStop; i++) {
				...
				if(!validAnswerOnly || actualIndex!=-1) {
					...
					if(actualParentNode.getClassificationNumber().isLeaf) {
						totalLeafNumber++;
					}
					else {
						totalNonLeafNumber++;
					...
					child.score = predictedParentNode.score;
