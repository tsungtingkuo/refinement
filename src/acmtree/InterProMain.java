...
public class InterProMain {
	...
	public static String run(int method, int features, boolean enrichment) throws Exception {
		...
		int testingMode = 0;				// 0=98, 1=pn, 2=doaj
		...
		InterProTraining.run(Integer.toString(method), alpha, beta, semanticThreshold, structuralThreshold, useAnd, singleFeature, upsampling, adjustSimilarity, possitiveThreshold, possitiveLevel, method, enrichment, features, cost);
		System.out.println("Training phase complete.");
					
		// Insertion
		System.out.println("Starting insertion phase...");
		String result = InterProTesting.run(Integer.toString(method), singleFeature, alpha, beta, gamma, semanticThreshold, newAsParent, method, validAnswerOnly, testingMode, keywordNumber, adjustScore, adjustSimilarity, view, suggestRank, 52670, enrichment, features);
