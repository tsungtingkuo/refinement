...
public class DOAJMain {
	...
	public static String run(int method, int features, boolean enrichment) throws Exception {
		...
		DOAJTraining.run(Integer.toString(method), alpha, beta, semanticThreshold, structuralThreshold, useAnd, singleFeature, upsampling, adjustSimilarity, possitiveThreshold, possitiveLevel, method, enrichment, features, cost);
		System.out.println("Training phase complete.");		
		
		// Insertion	
		System.out.println("Starting insertion phase...");
		String result = DOAJTesting.run(Integer.toString(method), singleFeature, alpha, beta, gamma, semanticThreshold, newAsParent, method, validAnswerOnly, testingMode, keywordNumber, adjustScore, adjustSimilarity, view, suggestRank, 1211100, enrichment, features);
