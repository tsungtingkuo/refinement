 ...
 *
 */
public class ACMTreeTrainingDataGeneration {
	...
	public static void main(String[] args) throws Exception{
		...
		ACMTreeTrainingDataGeneration.run(fileSuffix, alpha, beta, semanticThreshold, structuralThreshold, useAnd, singleFeature, upsampling, adjustSimilarity, possitiveThreshold, possitiveLevel, method, enrichment, features);
	}
	
	// For simplification
	public static int run(int method) throws Exception{
		...
		return run(fileSuffix, alpha, beta, semanticThreshold, structuralThreshold, useAnd, singleFeature, upsampling, adjustSimilarity, possitiveThreshold, possitiveLevel, method, enrichment, features);
	}

	public static int run(String fileSuffix, double alpha, double beta, double semanticThreshold, double structuralThreshold, boolean useAnd, boolean singleFeature, int upsampling, int adjustSimilarity, double possitiveThreshold, int possitiveLevel, int method, boolean enrichmentUsed, int featuresUsed) throws Exception{
		...
		if(method == 6 || method == 20 || method == 22 || method == 100 || method == 101 || method == 110 || method == 120 || method == 121 || method == 130 || method == 131) {
